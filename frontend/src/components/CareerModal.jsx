...
const CareerModal = ({ career, onClose, onEdit }) => {
    ...
    <div className="service-modal-overlay">
      <div className="service-modal-card">
        ...
        <div className="modal-body">
          ...
          <div className="modal-detail-group">
            ...
            <div style={{ display: 'flex', gap: '15px', marginBottom: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', color: '#666', fontSize: '14px' }}>
                <MdLabelOutline /> <strong>{career.category}</strong>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', color: '#666', fontSize: '14px' }}>
                <MdCalendarToday /> {career.eventDate || 'No date set'}
              </div>
            </div>
          ...
          <div className="modal-detail-group">
            ...
            <ul style={{ fontSize: '12px', color: '#888', listStyle: 'none', padding: 0 }}>
              <li><strong>Post Status:</strong> {career.status}</li>
              <li><strong>System Post Date:</strong> {new Date(career.datePosted).toLocaleString()}</li>
            </ul>
          </div>
        </div>
        ...
        <div className="modal-footer" style={{ display: 'flex', gap: '15px', marginTop: '20px' }}>
          ...
          <button className="btn-save" onClick={() => onEdit(career)}>
            Edit Post
          </button>
        </div>
